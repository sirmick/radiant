...
export function drawScene(ctx, S) {
  ...
  for (const m of S.marks) {
    ...
    else if (m.kind === 'wave') { ctx.translate(m.dx ?? 0, m.dy ?? 0); ctx.beginPath(); for (let i = 0; i < 6; i++) { const a = Math.PI / 6 + i * Math.PI / 3; ctx[i ? 'lineTo' : 'moveTo'](Math.cos(a) * m.size, Math.sin(a) * m.size); } ctx.closePath(); ctx.fillStyle = m.color; ctx.globalAlpha = m.recent ? 0.9 : 0.45; ctx.fill(); ctx.globalAlpha = 1; ctx.strokeStyle = '#0b0e13'; ctx.lineWidth = 0.8; ctx.stroke(); }
