<script>
  ...
  const fieldCtx = $derived.by(() => {
    ...
    const ctx = { powers: powersNow, colors: POWER_COLORS, lonlat, neKey, atWar: live.filter(atWar), intrastate: live.map(id => [id, intrastateOf(id)]).filter(([, l]) => l > 0), disputes: [...new Set(yearItems.filter(it => it.k === 'dispute' && it.a).flatMap(it => it.a))], actorHazards: [], dyadHazards: [], routes: [] };
  ...
  const hoverInfo = $derived.by(() => {
    ...
    const cellNote = cell && variable?.gradient ? (cell.mean ? `regime axis: ${cell.value.toFixed(2)} of 3, mean over runs` : cell.category ? `regime axis: category only (no polyarchy score${cell.stale > 0 ? `, as of ${cell.observed}` : ''})` : `polyarchy ${cell.poly.toFixed(2)} → ${cell.value.toFixed(2)} on the regime axis${cell.stale > 0 ? ` (as of ${cell.observed})` : ''}`) : cell && variable?.var !== 'regime' && variable?.kind === 'history' ? (cell.forecast ? `${variable.label}: ${d3.format(variable.display?.format ?? '.3~s')(cell.value)} median (${d3.format('.3~s')(cell.lo)}–${d3.format('.3~s')(cell.hi)})` : cell.stale > 0 ? `${variable.label}: ${d3.format(variable.display?.format ?? '.3~s')(cell.value)} as of ${cell.observed}` : '') : '';
...
<div class="map" bind:this={mapEl} bind:clientWidth={width} bind:clientHeight={height} role="application"
  ...
  <div class="legend">
    {#if variable}
      ...
      {/if}
      <div class="muted small">{variable.kind === 'forecast' ? (isForecastYear ? `${variable.template ? `P(within the next ${horizon} y from ${Math.round(year)}, given not yet) · ` : ''}ensemble of ${forecast?.meta.runs} runs` : 'before the forecast start: last observed state') : variable.kind === 'history' ? (isForecastYear ? (variable.gradient ? `after ${history.meta.y1}: the ensemble mean on the same axis · colour washes to grey as the runs spread out` : variable.var === 'regime' ? `after ${history.meta.y1}: the most likely state in each of ${forecast?.meta.runs} runs · colour washes to grey where the runs disagree` : `after ${history.meta.y1}: ensemble median where the engine carries the series, else the last observation · colour washes to grey with staleness`) : (variable.gradient ? `V-Dem polyarchy placed on the category axis by the panel's category medians (estimate) · where only the category is known it is painted flat and washed` : `historical panel · ${history?.meta.sources?.[variable.var] ?? ''} · a series that stopped early is carried forward and washes to grey`)) : 'modern snapshot'}</div>
    {/if}
    ...
    {#if layers.glyphs}<div class="lt" style="margin-top:6px">Qualities <span class="muted">percentile that year, clockwise from top: regime · GDP/cap · capability · info · urban</span></div>{/if}
  </div>
</div>
