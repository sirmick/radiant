<script>
  ...
  const marks = $derived.by(() => {
    ...
    if (layers.waves && waveNow) for (const [id, yr] of Object.entries(waveNow.sovereign_by ?? {})) { if (yr > Math.round(year)) continue; const ll = lonlat(id); if (!ll) continue; out.push({ lonlat: ll, kind: 'wave', size: 5, color: '#ffd166', dx: 9, dy: -9, recent: Math.round(year) - yr <= 5, title: `${history?.actors?.[id]?.name ?? id}: sovereign in ${waveNow.label} since ${yr}${waveNow.sovereign_source?.[id] ? ` — ${waveNow.sovereign_source[id]}` : ''}` }); }
