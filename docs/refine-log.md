...
**Checker, 2026-09-08 — verified independently, two fixes applied.** Rebuilt panel, events and fits from scratch: identical to the committed files except the build timestamp. Re-ran the 1870–2010 +20y 100-run backtest: all 15 pooled rows reproduce the committed file digit for digit and are identical to the pre-package baseline, so the layer costs nothing as claimed; `WAVE_CAPABILITY=1` re-run reproduces the rejection evidence exactly (mid_force AUC 0.773, mid_war 0.688, record_reopen skill −0.102). Tests (a), (b) and (c) re-run and reproduce. Test (a) additionally survives freezing `wave_diffusion` — the one covariate whose value comes from the held-out wave's own attainment dates — at the training mean (4/5 waves still over the bar, ρ 0.968 / 0.978 / 1.000 / 0.961 / 0.542), so the held-out result is not the wave's own answer fed back to it. Seam check: the engine's first simulated `wave_share` and the panel's observed one agree to within 2% for the majors (worst actor 0.16 in log).

Two things it found. (1) **A dead covariate in the engine**: `stepWaves` ran before the war-spell loop and the drift loop had already cleared `a.cur.at_war`, so `at_war` — fitted at +0.57 — was 0 for every actor in every simulated year while the fit read the panel's real value. The fit and the simulation were not the same model on that one term. Fixed in `src/engine/core.js` by reading the war state the actor enters the year with (`a.cur.at_war || a.prev?.at_war`); the event backtest is unchanged (nothing reads the wave share by default and the draw order is untouched) and the published ensembles are regenerated. (2) **The decomposition that decided against the substitution was not like-for-like**: `industry_share` is observed for 232 of the 322 post-1945 dispute-dyads, so its 0.628 was compared against a wave-share AUC measured on 322 rows. On the matched rows the order reverses (wave 0.660 against industry 0.628; 1946–2010 0.663 against 0.634). Corrected in `docs/escalations.md`, `docs/system.md` and the `rejected:` record itself. The rejection stands — it rests on the event backtest, which is the promotion rule — but its stated diagnosis does not.
