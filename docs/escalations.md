...
**Status:** **partial** (implemented 2026-09-08). (a), (b), (d) and (e) pass; (c) fails, and the failure is measured and diagnosed below rather than argued away.

**Implemented as.** `runEnsemble` takes a second flag beside `track`: `state: true` (`src/engine/core.js`). It reads the world after each `stepYear` and accumulates, per year offset — per actor, the runs it is live in, at war in, carrying internal conflict at level ≥ 1 and ≥ 2, and occupied in, plus its capability share into a per-run array; per dyad, the runs the pair is at war in *during* the year; per corridor, chokepoint and territory record, the count of each status word. It draws no random numbers, which is the whole reason it is a separate flag from `track`: the backtest can turn it on permanently and move nothing.

Three deviations from the package as written, all recorded here:

- **The dyadic at-war set is built inside `stepYear`, not reconstructed by the caller.** `world.warSpells` holds the spells still open when the year *closes*; a spell that ends in the year, and a war that fires and ends in the same year, were at war during it and never appear there. `stepYear` now publishes `world.atWarPairs` — the spells it iterated plus this year's onsets and the pairs a coalition draw opened — and the tracking reads that.
- **Two capability shares, not one.** The package names `cinc`. Nothing in the engine rewrites `cinc`, so its p10–p90 band is degenerate and the series is the as-of ranking held still; publishing only that would paint a frozen number as a forecast. `pol_share`, the projection-weighted share `src/engine/polarity.js` advances by each actor's own simulated output and population growth, is the one that moves. Both are carried, both are flagged `simulated: true|false` in `meta.state.vars`, and the rank-correlation test is reported for both.
- **`intrastate_war` and `occupied` are published and scored, and labelled as floors rather than forecasts.** The onset template has no intensity, so the engine writes internal conflict at level 1 and never 2 — every unit of `intrastate_war` mass is a level-2 spell the world entered with. Occupation is data in this model, not a hazard, so `occupied` is the as-of value held still. Both carry a `note` saying exactly that, because a silent zero on either would read as a calibrated forecast.

Two things the package did not ask for and the score needed: the record layer is graded **twice** — as a binary "impaired" (the quantity the map shows) and as the full multi-class status Brier, Σ_c (p_c − 1{c = actual})² against the marginal status distribution of the scored rows — and the territory layer the same way as `territory_unsettled` plus its own multi-class row. A single modal word would have thrown away the distribution the block exists to carry.

`scripts/run-forward.mjs` writes `state.actors[id][var][k]`, `state.dyads[pair][k]` and `state.records[id][k]` with `meta.state.vars` naming every series, its `simulated` flag and its caveat. The dyad block keeps every pair whose peak year reaches 0.02, topped up to at least 500 pairs by peak and capped at 2,000, with the lowest kept peak and the dropped count in `meta.state` — a missing pair is a stated omission, not a silent zero; a series that is zero in every run of every year is written `null` for the same reason. `scripts/backtest.mjs` scores the block at every rolling origin under `byAsOf[].occupancy` and top-level `occupancy`; `scripts/build-scores-slice.mjs` carries it to `public/scores.json` and the Scores tab shows it as a second table with a per-lead-year exp/obs bar per variable.

**Test result.**

**(a) The block, and its cost.** `node scripts/run-forward.mjs --runs 20 --horizon 40`: **195 actors** (bar: ≥ 190), **620 dyads** (bar: ≥ 500), **157 records** — which is *every* open record, checked against the layer rather than counted: 61 corridor/chokepoint records live at as-of 2025 and 96 territories whose history has opened, 157 in the block, 0 missing, 0 extra. At 300 runs × 100 years `public/forecast.json` goes 4.07 MB → **5.94 MB, a factor of 1.459** (bar: < 2×), of which the state block is 1.87 MB. The floor rule is what the run count costs: at 200 runs a pair at war in 3 of them peaks at 0.015 and would fall under the 0.02 threshold, so the six past-as-of ensembles keep 500 pairs each with lowest kept peaks of 0.005–0.015.

**(b) Occupancy Brier at as-of 1955 / 1975 / 1990** (100 runs, +20, `--from 1955 --to 1990 --step 5`, `scores/backtest-1955-1990-h20-all.json`). The number exists and is honest; the package did not require skill and does not get much:

| as-of | at_war n | rate | exp/obs | Brier | base | skill | dyad_at_war exp/obs | intrastate skill | record_impaired exp/obs |
|---|---|---|---|---|---|---|---|---|---|
| 1955 | 2,461 | 5.12% | 0.91 | 0.0502 | 0.0486 | −0.03 | 0.72 | +0.01 | 1.07 |
| 1975 | 3,294 | 2.97% | 0.65 | 0.0283 | 0.0289 | +0.02 | 0.71 | +0.28 | 1.21 |
| 1990 | 3,789 | 1.35% | 0.92 | 0.0133 | 0.0133 | +0.00 | 0.41 | +0.24 | 3.34 |

Pooled over the eight origins 1955–1990: `at_war` n=25,433 exp/obs 0.68 skill +0.014; `intrastate` n=25,433 exp/obs 0.97 skill **+0.218**; `dyad_at_war` n=2,055,691 exp/obs 0.52 skill −0.002; `record_impaired` n=6,295 exp/obs 1.21 skill +0.07; `territory_unsettled` n=11,720 exp/obs 0.87 skill **+0.803**; multi-class status Brier 0.155 against a 0.575 base (skill +0.73) for corridors and 0.191 against 0.564 (+0.66) for territories. Capability rank correlation, mean over the eight origins: `cinc` ρ = 0.929 at k=10 and 0.872 at k=20, `pol_share` ρ = 0.956 and 0.938 — the projected share ranks the world slightly *better* twenty years out than the frozen index does, which is the first evidence the polarity projection is worth anything. Over the published 1870–2010 run, the same numbers are `at_war` 0.65 / +0.02, `intrastate` 1.09 / +0.19, `dyad_at_war` 0.28 / +0.00, `record_impaired` 1.58 / −0.05, `territory_unsettled` 1.00 / +0.71, and `cinc` ρ 0.945 / 0.891 with `pol_share` 0.950 / 0.936.

**The hot-process diagnosis, which is what the score was built to see, and it is in the log.** The simulated war occupancy exceeds the panel's at as-of 1955 at lead years 2–9 and 13 (worst k=5: 4.00% of actor-years simulated at war against **0.00%** observed); at as-of 1975 at leads 1 and 17–20 (worst k=18: 1.67% against 0.00%); at as-of 1990 at leads 3–5, 7, 12 and 14–17, 19–20 (worst k=16: 1.47% against 0.00%). The shape is the same in all three and is not a level error — pooled exp/obs is *under* 1 everywhere — it is that **the simulated at-war rate is flat in lead while the observed one decays**. At as-of 1990 the ensemble holds 1.0–1.5% of states at war in every one of years 2004–2010 while the panel records none (checked against the panel directly: 0, 0, 4, 0 at-war actor-years in 2004, 2005, 2008, 2010 out of 192–194 live states — the modern zero is real, not a coverage hole). The dyadic half says the same at leads 4–20. That is the war process running long, and it is the first statistic in the model that can see it: the event score's `mid_war` row at these origins is a *first-occurrence* number and reads 0.72 pooled.

**(c) Suez at as-of 1955 — FAILS, and the number is the finding.** `public/forecast-1955.json` (200 runs, +40) gives `state.records.suez` a closed-or-contested mass of **5.0% in 1956 and 9.5% in 1957** (window mean 7.25%) and **22.72% over 1967–75**, against the record's own observed impaired rate of **16.9% over 1955–2025** (12 of 71 years: 1956, 1967–74, 2023–25) and the ensemble's own horizon mean of **22.06%**. So 1967–75 clears the 1955–2025 mean and 1956–57 does not clear anything — it is the *lowest* part of the whole horizon. The mass is not timed; it accumulates monotonically with lead, from 5% in year 1 to a plateau near 22% by year 10, and 1967–75 is above the historical rate only because it sits on that plateau. The diagnosis is not a bug: nothing in the world at the end of 1955 tells the chokepoint hazard that Egypt nationalises the canal in July 1956, and the covariates it does have (`adjacent_war`, `transit_at_war_any`, `record_building`, the presence terms) fire only when a simulated war reaches Egypt, which happens in 5% of runs at lead 1. What the test does establish is the aggregate calibration nobody had measured: over 1956–1995 the ensemble's mean impaired mass is **22.06%** against the record's observed **22.5%** on the same forty years — right to half a point in level, and wrong in every year. A record-layer hazard that is calibrated in aggregate and blind in timing is exactly what the corridor templates' 0.64–0.66 pooled AUC already implied, and this is the first statement of it in the units the map paints.

**(d) The event backtest is unchanged — byte-identical, not "within stream noise".** The tracking consumes no random numbers, so the claim can be made exactly rather than statistically, and it was checked twice. (i) At as-of 1955 and 1990, 30 runs × 20 years, `runEnsemble` with `state: true` and without produce identical `pAny`, `expected`, `pAnyDyad`, `yearHist` and `firstBy` (1,218 / 1,509 dyad keys, 1,050 / 1,372 actor keys). (ii) The full `--from 1870 --to 2010 --step 10 --horizon 20 --runs 100 --universe all` run was executed against a clean `git archive HEAD` checkout of the repository and against this branch: the two `scores/backtest-1870-2010-h20-all.json` files are **character-identical** (114,164 bytes each) once `meta.run`, `byAsOf[].ms` and the new `occupancy` blocks are removed. (The `scores/` file previously committed is *not* a valid comparison for this — it dates from `b68e730`, before the era-modern turn moved `COVERAGE.coup` to 2025, and differs from HEAD's own output.)

**(e) Viewer.** Conflict at 2036 paints the belligerents field, the country outlines and the dyadic arcs from `state` — before this the field was literally empty after the seam (`live.filter(atWar)` returned nothing once the panel ran out) and the outlines were absent. Routes at 2036 draws each corridor and chokepoint at its modal simulated status washed by its probability, and the hover card gives the mix. The Scores tab carries the occupancy table with the per-lead-year exp/obs bars. Screenshots via `node scripts/shot2.mjs out.png 2036 "Conflict"` / `"Routes"`, and `node scripts/analysis/shot-panel.mjs out.png "#y=2036&view=conflict" Scores 880` for the panel below the fold (new: `shot2.mjs` cannot scroll the panel). `docs/ui.md`, `docs/schema.md`, `docs/runbook.md` and `docs/system.md` describe the block and the score.

One file outside the operator's list was touched and is flagged here: `src/lib/influence.js`. The belligerents field's `conflict` source read `ctx.atWar` as a plain id list and `ctx.intrastate` as `[id, level]` with a hand-typed 0.6 / 1.0 by intensity. Both are now `[id, weight]`, where the weight is the panel's flag in history and the ensemble's P(at war) / expected internal-conflict weight after the seam. The field could not be made probability-weighted without it, and leaving the ctx shape changed on one side only would have silently zeroed the interstate half.

One arc threshold is worth stating because it is a finding rather than a style choice: the arcs draw at P ≥ 0.01, not 0.05, because **the strongest pair-year in the whole 2026–2125 ensemble peaks under 0.02**. The engine spreads its war mass across a great many pairs and concentrates it on none; a 5% floor drew nothing at all. Alpha is the probability itself and is deliberately not rescaled.

**Scores: before → after.** The event score is unchanged by construction (see (d)), so the templates this package feeds move by nothing: pooled 1870–2010 stays `mid_force` 0.82 · +0.11 · 0.81, `mid_war` 0.72 · +0.03 · 0.78, `coup_attempt` 0.89 · +0.23 · 0.79, `intrastate_onset` 0.98 · +0.23 · 0.79, `chokepoint_status` 1.17 · +0.04 · 0.64, `corridor_status` 1.39 · +0.04 · 0.66, `contest_settle` 1.08 · +0.11 · 0.80. What is added is a second score with no "before": the occupancy rows above.

**What the next package should know.** The hot-process number now exists and is the guard `era-1914-1945 / engine-5` (war duration) has to move: `at_war` occupancy is flat in lead where the world's decays, and it is visible per lead year at every rolling origin. `intrastate_war` and `occupied` are scored floors, not forecasts, and will stay so until internal conflict gets an intensity and occupation gets a hazard; both are named in `meta.state.vars` so nothing downstream can read them as model output. `cinc` does not move in the engine at all — anything that wants a capability *forecast* has to go through `pol_share` or replace `cinc` (which is `operator / capability-waves`, package 12).
...
**Status:** implemented 2026-09-08. Tests (a), (b), (c) and (d) all pass as written; the *capability substitution* the package proposed behind the switch is measured and recorded as **rejected** by the event backtest, so it stays a candidate and CINC keeps the dyadic ratio. The wave layer itself — the data, the fitted attainment hazard, the published share — is on by default and costs the event backtest nothing.

**Implemented as.**

- **`data/waves.yaml`** — 14 → **19 waves**. Five split out because the operator's question needs the mass dimension to have both ends: `ballistic_missiles` (1944), `precision_strike` (1972), `stealth` (1981), `armed_drones` (2001), `hypersonics` (2019). Every wave now carries `mass` in [0,1] with a `mass_source` (stealth 0.1 — fifty-nine airframes flew the 1991 campaign; armed drones 0.9 and solar/battery 0.95 — the war is the factory; steel 1.0 — the capability and the output series are the same number; nuclear 0.15 — possession is nearly the whole of it). Every wave introduced from 1900 carries a `sovereign_source` **per `sovereign_by` entry**, citation or `estimate` naming what the judgement was made on; `src/engine/waves.js` `loadWaves()` fails the build if one is missing (`DOCUMENTED_FROM = 1900`; the pre-1900 waves stay under the file header's blanket Bairoch/Mitchell note). The `hypersonics` wave has no US entry through 2024 on purpose: an index that cannot show the leading military spender missing a wave is not measuring waves.
- **`src/engine/waves.js`** (new, no country names) — the whole construction in one module, imported by `scripts/build-panel.mjs`, `scripts/lib/fit.mjs`, `src/engine/core.js` and `scripts/analysis/waves.mjs`, so the fit and the simulation are one object. `waveWeight` is the displacement schedule (a logistic on the wave's *own* diffusion clock — half weight at `median_years_to_S` — times a 20-year-half-life decay after `retired`, so no second set of numbers is invented); `waveShares` is the index; `attainFeatures` / `attainRows` build the hazard's covariates and sample.
- **Panel columns** — `industry_share` (and `industry_share_carried`), `wave_share`, `wave_attained`. The industrial share is built in **two stages** rather than as a chain of per-actor splice factors, because a share computed inside one source's coverage is a share of *that source's* world: the World Bank manufacturing series covers 19 live actors in 1960 against NMC's 107, and the naive splice handed one of those 19 a sixth of world industry. Stage 1 is iron and steel, else primary energy, else electricity generation, each rescaled onto the steel scale by that year's median ratio and normalised to a world share; stage 2 lets the manufacturing block redistribute **its own** stage-1 aggregate. What is carried between years is each member's *relative weight inside the block*, not its dollar figure — the World Bank stops publishing US manufacturing value added after 2021 while it keeps publishing everyone else's, and without that the largest OECD manufacturer leaves the block in 2022 and its share falls 15.8% → 4.3% on a source seam.
- **`wave_attain`** — a new template, unit `actor-wave-year`, `lead: 1`, window 1825–2024. n = 65,879, 153 attainments (0.23%), in-sample AUC 0.907, era-holdout (≥1945) 0.639. Covariates: the diffusion clock, log industrial share, income, a defence pact with an actor already sovereign **in this wave**, the epidemic term, war, great-power status. Its at-risk floor is declared and swept: the actor must hold ≥ 0.03% of world industrial output (a stand-in for the ladder's level I, which no source gives). Without the floor the sample is 167,506 rows and 175 attainments (0.10%), nine tenths of them states that were never candidates; the **22 attainments below the floor are structural misses** and `scripts/analysis/waves.mjs` prints them rather than hiding them.
- **The engine** — `stepWaves` runs before `stepPolarity`: industrial mass drifts on each actor's own simulated output and population growth (the same two terms `stepPolarity` uses), every at-risk actor-wave draws the fitted hazard, and the share is rewritten on every actor state. The draws come from **their own random stream** (`waveRng`, seeded per run in `runEnsemble`), which is what makes the ablation a measurement of the mechanism rather than of the stream.
- **Two switches, not one.** `ENGINE_ABLATE=waves` removes the layer entirely. `WAVE_CAPABILITY=1` (or `capability.status: active` on the template) additionally makes the dyadic capability ratio and the termination layer read the wave share in place of CINC. Publishing a share as a forecast and letting it decide a war are different claims; only the first is on.
- **Published** — `state.actors[id].wave_share` / `wave_attained` / `industry_share`, p10/p50/p90 per lead year, all `simulated: true`, beside the unchanged `cinc` (still flagged `simulated: false`). `meta.waves` says whether the substitution was on when the file was written. The backtest grades `wave_share` against the panel's **own** wave share, not against CINC — grading it against CINC would score it on how well it reproduces the index it replaces. The viewer's **Industry** and **Power** views now paint `h_wave_share`; `h_cinc`, `h_industry_share` and the raw series stay in the picker, and the ⬢ wave marks carry each attainment date's source in the hover text.
- **Deviation from the package.** The polarity substitution is *measured* (test (c)) but **not wired**: `src/engine/polarity.js` is read by `scripts/build-panel.mjs` to write the era flags every era-interacted coefficient was fitted on, so swapping its capability input forward-only would put the fit and the simulation on two different definitions of `cold_war` and `promotion_era` — exactly the failure mode this project's own rules exist to prevent. Doing it properly is a panel rebuild plus a refit of every era term, and it is escalated below rather than smuggled in.

**Test result.**

*(a) Attainment timing, held-out waves* — fit on the five waves introduced before 1900 (23,859 rows, 52 attainments), predict the 20th-century ones. Predicted year is E[min(T, window end)] from the hazard path, which needs no threshold. Spearman ρ of predicted against actual attainment year, and the share whose decade is right:

| wave | n actors dated | ρ | decade right |
|---|---|---|---|
| aviation | 16 | **0.981** | 94% |
| radio_radar | 15 | **0.978** | 87% |
| nuclear | 9 | **1.000** | 44% |
| space | 11 | **0.975** | 55% |
| semiconductors | 10 | 0.542 | 90% |

**4 of 5 over ρ ≥ 0.6; the bar was 3 of 5. PASS.** Semiconductors is the miss and the interesting one: the pre-1900 waves say industrial base and great-power status decide attainment, and the semiconductor wave was attained by three small states (one city-state) out of order. The diffusion covariate is common to every actor in a year, so it cannot move a within-wave ranking — the ranking is doing the work.

*(b) Outcomes* — GML MID 2.2.1 decisive outcomes (codes 1–4), one undirected row per dispute-pair at its first observed year, capability read the year **before** the dispute. AUC of "the stronger side prevails": 1,015 decisive dispute-dyads, side 1 prevails in 55.3%.

| window | n | CINC | wave_share | Δ | *industry_share* | *milex* | *wave_attained* |
|---|---|---|---|---|---|---|---|
| 1900–2001 | 715 | 0.512 | **0.603** | **+0.092** | 0.575 | 0.515 | 0.451 |
| 1900–1945 | 393 | 0.507 | 0.596 | +0.089 | 0.523 | 0.481 | 0.443 |
| 1946–2001 | 322 | 0.517 | **0.612** | **+0.095** | **0.628** | 0.553 | 0.529 |
| 1946–2010 | 335 | 0.513 | 0.616 | +0.104 | 0.634 | 0.552 | 0.558 |
...
| template (pooled) | baseline (pre-package) | waves on, default | `WAVE_CAPABILITY=1` |
|---|---|---|---|
| mid_force | 0.0111 / AUC 0.810 / skill +0.113 | **identical** | 0.0116 / 0.773 / +0.074 |
| mid_war | 0.0052 / 0.780 / +0.032 | **identical** | 0.0055 / 0.688 / −0.016 |
| interstate_onset | 0.0008 / 0.889 / −0.046 | **identical** | 0.0008 / 0.812 / −0.051 |
| record_reopen | 0.1865 / 0.724 / −0.300 | **identical** | 0.1582 / 0.707 / **−0.102** |
| corridor_status | 0.2104 / 0.655 / +0.036 | **identical** | 0.1998 / 0.639 / **+0.085** |
| chokepoint_status | 0.2403 / 0.636 / +0.037 | **identical** | 0.2358 / 0.613 / **+0.055** |
| coup_attempt / intrastate_onset / the other 8 | — | **identical** | ±0.007 skill |

All **15** pooled template rows are **identical to the pre-package baseline** with the layer on by default — not "within stream noise", the same numbers — because the attainment draws come from their own stream and nothing reads the wave share. `ENGINE_ABLATE=waves` reproduces them too. Occupancy adds `wave_share_k10` ρ = 0.899 and `wave_share_k20` ρ = 0.817 against the panel's own wave share (`cinc` 0.945 / 0.891, `pol_share` 0.950 / 0.936 against CoW's), unchanged by the substitution.

**The decision the numbers force.** The substitution passes the package's outcome test and fails the model's own promotion rule. It is better on everything that reads capability through the *termination* layer and worse on every *dyadic* template, which is where the ratio actually does its work — and test (b)'s decomposition says why: the mass and displacement weights add nothing over the industrial share they are built on. So the **mass and displacement weights are recorded as rejected for the ratio** (`data/templates.yaml`, `wave_attain.rejected: wave_capability_substitution`, with every number and the reproduce command), the attainment template stays — (a) passes on its own, which is what the operator's note asked for — and the wave share is published as a forecast rather than used as a decider. Two things the re-proposal has to do: fit the mass coefficients instead of typing them, and read a blend rather than a swap.

**Scores: before → after.** Every event template pooled 1870–2010: **unchanged** (identical). New rows: `wave_attain` n=65,879 / 153 events / in-sample AUC 0.907 / holdout(≥1945) 0.639, reported in the event backtest with `n: 0` and the reason that its ground truth is `data/waves.yaml` rather than an event kind. New occupancy rows: `wave_share_k10` 0.899, `wave_share_k20` 0.817.

**What the next package should know.** Three things are now open and named. (1) **Polarity on the wave index** is measured and unwired; doing it needs `scripts/build-panel.mjs` to write the era flags from the chosen capability definition and every era-interacted coefficient refitted against it, which is a package, not an edit. (2) **The mass coefficients are hand-typed claims** — nineteen numbers with `mass_source` each, and test (b) says they are not earning their place against the raw industrial share. Fitting them needs a wave-level outcome sample (which side's *waves* decided a dispute) that no source in `data/raw` carries. (3) **US manufacturing value added ends in 2021** in WDI; `industry_share_carried` counts the carry (179 block weights carried, 206 whole shares) and the fix is UNIDO or BEA, still not fetched. Also: `import_value` is 0 — an actor that buys a whole inventory contributes nothing to its own wave share — which is the assumption in this layer most likely to be wrong.
