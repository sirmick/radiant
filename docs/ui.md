...
- **capability, the wave index** (`operator/capability-waves`): `wave_share`, `wave_attained` and `industry_share` are carried beside those two and are simulated. `wave_share` is capability as a portfolio of dated technology waves rather than steel-energy-soldiers (`src/engine/waves.js`); its p10–p90 band is genuinely wide, because an actor that attains a wave in one run and not in another separates the quantiles, where `cinc`'s band is a single frozen number. `wave_attained` is the attainment half alone, before any mass discount, and `industry_share` is the industrial mass the waves are weighted by, carried forward by each actor's own simulated growth.
