...
// Run: node scripts/fit-hazards.mjs [template_id ...]
...
// pair-level defence pacts (1816–2000)
const pacts = new Set();
...
function buildDyadRows(t) {
  const rows = []; const [w0, w1] = t.window; const ids = Object.keys(panel.actors);
  for (let y = Math.max(w0, Y0 + 5); y <= Math.min(w1, panel.meta.y1); y++) {
    ...
    for (let i = 0; i < live.length; i++) for (let j = i + 1; j < live.length; j++) {
      ...
      rows.push({ unit: pairKey(a, b), year: y, feats, y: hasDyadEvent(t.event, a, b, y) ? 1 : 0 });
    }
  }
  return rows;
}
...
function fitLogistic(X, y, prior, lambda = 2.0, iters = 50) {
...
const predict = (X, beta) => X.map(xi => 1 / (1 + Math.exp(-xi.reduce((s, x, j) => s + x * beta[j], 0))));
function auc(p, y) {
...
function calibration(p, y, bins = 10) {
...
for (const t of templates) {
  ...
  if (t.status === 'candidate' || ['chokepoint-year', 'corridor-year'].includes(t.unit) || t.sample === 'latent_only') { fits[t.id] = { status: 'unfitted', reason: 'no panel sample yet; using literature prior' }; lines.push(`${t.id.padEnd(24)} unfitted (prior only)`); continue; }
  const rows = t.unit === 'dyad-year' ? buildDyadRows(t) : buildActorRows(t);
  if (rows.length < 50 || rows.filter(r => r.y).length < 5) { fits[t.id] = { status: 'unfitted', reason: `n=${rows.length}, events=${rows.filter(r => r.y).length}` }; lines.push(`${t.id.padEnd(24)} unfitted (${fits[t.id].reason})`); continue; }
