...
// Run: node scripts/run-forward.mjs [--runs 500] [--horizon 40] [--universe all|modeled]
...
const asOf = panel.meta.y1;   // 2025: the last panel year
...
const out = { meta: { built: new Date().toISOString(), asOf, from: asOf + 1, to: asOf + H, runs: RUNS, horizon: H, universe: UNIVERSE, engine: 'generic templates only — named 2026 hazards (Taiwan, Hormuz, CRQC…) not yet wired', templates: simulated.map(t => ({ id: t.id, label: t.label, unit: t.unit })) }, actors: {}, dyads: {} };
for (const id of ids) {
  const p = {};
...
writeFileSync('public/forecast.json', JSON.stringify(out));
console.log(`forecast.json: ${ids.length} actors, ${Object.keys(out.dyads).length} dyads ≥5%, ${RUNS} runs × ${H}y in ${((Date.now() - t0) / 1000).toFixed(0)}s, ${(JSON.stringify(out).length / 1024).toFixed(0)} KB`);
// headline table
